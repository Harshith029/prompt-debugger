...
- The flagship scenario — a visible model-switch notice after a safeguard decline (a request visibly answered by a different, fallback model) — is `kind: "model_switch"` with the surface the user reported. The *citation* attached via the taxonomy stays within documented API mechanisms; the taxonomy entry records what can and cannot be concluded for each surface. Provider-specific detail (which models, which product surfaces) lives in the provider knowledge pack, not in this contract.
